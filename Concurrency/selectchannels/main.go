...
func logMessages(chEmails, chSms chan string) {
	// ?
	for {
		/* listens to channels chEmails and chSms at the same time */
		/* body of the first channel with a value will exexute */
		
		/* one chosen randomly if both have a value at the same time
		 @todo - Find out what happens to other value when one channel is chosen randomly */ 
		select {
